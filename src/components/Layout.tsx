...
export default function Layout({
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-5xl flex-col px-4 py-6 md:px-8 md:py-10">
      {showHeader ? (
        <header className="rounded-2xl border border-(--line) bg-(--panel) px-5 py-4 shadow-[0_8px_24px_rgba(0,0,0,0.05)] md:px-7 md:py-5">
      ...
      <main
        className={`flex-1 rounded-2xl border border-(--line) bg-(--panel) p-4 md:p-6 ${
          showHeader ? "mt-6" : ""
        }`}
      >
        {children}
